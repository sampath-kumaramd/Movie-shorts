...
export const ActorForm: React.FC<ActorFormProps> = ({
  ...
  const onSubmit = async (data: ActorFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/actors/${params.actorId}`, data);
      } else {
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/actors/${params.actorId}`);
      router.refresh();
      router.push(`/actors`);
      toast.success('Actor deleted.');
    } catch (error: any) {
      toast.error('Make sure you removed all films using this actor first.');
    } finally {
      setLoading(false);
      setOpen(false);
    }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
          <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Actor image</FormLabel>
                  <FormControl>
                    <ImageUpload 
