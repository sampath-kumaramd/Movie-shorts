...
function Row({ title, movies }: Props) {
  ...
  return (
    <div className="h-full space-y-0.5 md:space-y-2">
      <h2 className="cursor-pointer text-sm font-semibold text-[#000000] transition duration-200 hover:text-white md:text-2xl">
        {title}
      </h2>
      <div className="group relative md:-ml-2">
        ...
        >
          {movies.map((movie) => (
            <Thumbnail key={(movie.id as Key)} movie={movie} />
          ))}
        </div>
